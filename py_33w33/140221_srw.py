...
#thetas = np.linspace((0.0001)*np.pi,2.89,25)
dt = 1.0
...
A_tot = np.zeros(shape=(len(ls),len(thetas)))
...
for k,length in enumerate(ls):
	for i, theta in enumerate(thetas):
		...
		A_tot[k,i] = sum_A[k] + sum_A_2[k]* np.cos(2.0*theta)
...
pl.legend(loc = 'best')
pl.title(r'$\mathrm{CNT [3,3]\,}$', size = 20)
...
pl.legend(loc = 'best')
pl.title(r'$\mathrm{Anisotropy \,Metric\, for CNT [3,3] and water}$', size = 20)
#pl.axis([0,500,0.9,2.6])
...
pl.loglog(ls,(kb*T/32)*sum_A_2,'g--', label = r'$\mathcal{A^{(2)}}$')
pl.loglog(ls,(kb*T/32)*A_tot,'r--')#, label = r'$\mathcal{A}$')
...
ax_inset = fig.add_axes([0.53,0.50,0.36,0.36])
#ax_inset.semilogy(thetas, G_l_t_dt[12,:], label = r'$\ell$\,=\,4.0\,nm')
...
pl.ylabel(r'$-G(\ell,\theta)\,\,\mathrm{[k_{B}T]}$', size = 20)
#pl.axis([1.9e-9, 8.1e-9,1e-2,1e2])
