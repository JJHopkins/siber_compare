...
for j,n in enumerate(ns):
	#f = (1./np.sqrt(Y**2 -1.0))*np.exp(-2.*Y*p[j]*np.sqrt(T*T+1))*(T/np.sqrt(T*T+1))*(2.*(1.-3.*a[j])*(1.-3.*a[j])*T**4 + 4.*(1.+2.*a[j]+2.*a[j]+3*a[j]*a[j])*T**2 + 4.*(1.+a[j])*(1.+a[j]) +  (T**4 + 4.*T**2+4.)*(1.-a[j])*(1.-a[j]) )
	f = (1./np.sqrt(Y**2 -1.0))*(T/np.sqrt(T*T+1))*(2.*(1.-3.*a[j])*(1.-3.*a[j])*T**4 + 4.*(1.+2.*a[j]+2.*a[j]+3*a[j]*a[j])*T**2 + 4.*(1.+a[j])*(1.+a[j]) +  (T**4 + 4.*T**2+4.)*(1.-a[j])*(1.-a[j])) * (1.0 - 2.0*(-2.*Y*p[j]*np.sqrt(T*T+1)) + 2.0*(-2.*Y*p[j]*np.sqrt(T*T+1))*(-2.*Y*p[j]*np.sqrt(T*T+1)) - (4./3)*(-2.*Y*p[j]*np.sqrt(T*T+1))*(-2.*Y*p[j]*np.sqrt(T*T+1))*(-2.*Y*p[j]*np.sqrt(T*T+1)) + (2./3)*(-2.*Y*p[j]*np.sqrt(T*T+1))*(-2.*Y*p[j]*np.sqrt(T*T+1))*(-2.*Y*p[j]*np.sqrt(T*T+1))*(-2.*Y*p[j]*np.sqrt(T*T+1)))
	Ft = np.sum(f,axis = 0)
	Fty = np.sum(Ft)
	print j,Fty
