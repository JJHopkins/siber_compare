...
#pl.semilogy(thetas, G_l_t_dt[0,:], label = r'$\ell$ = %3.1f nm' %(1e9*ls[0]))
pl.semilogy(thetas, G_l_t_dt[1,:], label = r'$\ell$ = %3.1f nm' %(1e9*ls[1]))
pl.semilogy(thetas, G_l_t_dt[2,:], label = r'$\ell$ = %3.1f nm' %(1e9*ls[2]))
pl.semilogy(thetas, G_l_t_dt[3,:], label = r'$\ell$ = %3.1f nm' %(1e9*ls[3]))
...
pl.grid(which = 'both')
pl.tick_params(which = 'both',labelright = True)
pl.savefig('plots/skew_ret_water/140306_65w65_skew_ret_G_vs_theta.pdf')
show()
