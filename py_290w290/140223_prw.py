...
#NOTE: the z and x directions are switched here because otherwise, they give a really weird aiz
# its hard to tell which eiz direction is par and which is perp
...
#aiz = np.absolute(Aiz(eiz_x,eiz_z, eiz_w)) 
...
for k,length in enumerate(ls):
	...
	A[0,k] = (1./2)*A[0,k] #NOTE fix me!! 		
	A_2[0,k] = (1./2)*A_2[0,k]  #NOTE fix me!		
...
for k,length in enumerate(ls):
	...
	G_l_t_dt[k] = (1.602e-19 / 4.11e-21) * (1./32) * EL[k]*r_1*r_1*r_2*r_2*(sum_A[k] + sum_A_2[k])
...
pl.savefig('plots/par_ret_water/140306_290w290_par_ret_A.pdf')
show()
...
pl.xlabel(r'$\ell\,\,\mathrm{[m]}$', size = 20)
...
pl.grid(which = 'both')
pl.tick_params(which = 'both',labelright = True)
pl.savefig('plots/par_ret_water/140306_290w290_par_ret_g_vs_l.pdf')
show()
