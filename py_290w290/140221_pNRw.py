...
G_srw = np.loadtxt('g_prw.txt') #perpendicular, radial
...
for j,n in enumerate(ns):
	...
	A[0] = (1./2)*A[1] #NOTE fix me!! 		
	A_2[0] = (1./2)*A_2[1]  #NOTE fix me!		
...
for k,length in enumerate(ls):
	...
	G_l_t_dt[k] = (1.602e-19 / 4.11e-21) * (9.*np.pi/(32.*64.)) * EL[k]*np.pi*r_1*r_1*r_2*r_2*(sum_A + sum_A_2)
	#G_l_t_dt[k] = (1.602e-19 / 4.11e-21) * (1./32) * EL[k]*np.pi*r_1*r_1*r_2*r_2*(sum_A + sum_A_2)
...
pl.figure()
pl.loglog(ls, G_l_t_dt, label = r'$\mathrm{\mathcal{A}\,=\,}%5.2f$'%(1.0e21*(3.*kb*Temp/2)*(sum_A+sum_A_2)))
...
pl.savefig('plots/par_NR_water/140306_290w290_Nonret_skew_g_vs_l.pdf')
pl.show()
...
pl.figure()
#pl.loglog(ls_power,l_6/l_6[0],'b--', label = r'$\ell^{-4}$')
...
pl.savefig('plots/par_NR_water/140306_290w290_Compare_par_normalized_g_vs_l.pdf')
pl.show()
